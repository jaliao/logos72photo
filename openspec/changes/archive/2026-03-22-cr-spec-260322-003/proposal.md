...
背景圖層加上 `opacity: 0.1` 後底紋幾乎不可見，效果不如預期，需取消。同時個人相簿頁（`/album/*`）需要使用專屬背景圖 `/bg/album/1.png`，與後台管理頁的響應式背景區分開來。
...
- `app/album/login/page.tsx` 與 `app/album/[slotGroup]/page.tsx` 傳入 `bgSrc="/bg/album/1.png"`
