...
### Requirement: 影像處理路由
Image-service Worker SHALL 提供統一的影像處理端點，接受 width、quality 與 r2_key 參數，回傳處理後的 WebP 圖片。

#### Scenario: 正常請求回傳 WebP 縮圖
- **WHEN** 用戶端發送 `GET /resizing/{width}/{quality}/{r2_key}`
- **THEN** 系統 SHALL 回傳 Content-Type 為 `image/webp` 的圖片，尺寸為指定寬度（高度等比例）
...
- **WHEN** L1 未命中，但 R2 `thumbnails/{width}w_{quality}q/{r2_key}.webp` 存在
...
### Requirement: WebP 格式輸出
系統 SHALL 以 WebP 格式輸出所有處理結果，quality 參數控制壓縮品質。

#### Scenario: 輸出 WebP
- **WHEN** 影像處理完成
- **THEN** 輸出 MUST 為 WebP 格式，Content-Type: `image/webp`

#### Scenario: quality 參數生效
- **WHEN** quality=80
- **THEN** 輸出 WebP 品質 SHALL 對應 80%，檔案大小應明顯小於 quality=100
