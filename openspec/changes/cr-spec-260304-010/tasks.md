...
- [ ] 7.1 在 iPhone Safari 開啟相機頁面，確認整 5 分鐘時自動進入倒數並拍照
- [ ] 7.2 關閉 Firebase RTDB 連線（或斷網），確認拍照仍在整點正常觸發
- [ ] 7.3 確認狀態列「時差」欄位在收到 `/api/trigger` 後 5 秒內更新
- [ ] 7.4 確認頁面重新載入後定時器正確重設，不在非整點時刻拍照
